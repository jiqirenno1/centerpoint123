...
def main():
    cfg = Config.fromfile(
        '../configs/mykitti/centerpoint_pp_02voxel_two_pfn_onnx.py')
    ...
    checkpoint = load_checkpoint(model,
                                 '../work_dirs/centerpoint_pp_02voxel_two_pfn/latest.pth',
                                 map_location="cpu")
    model.eval()
    ...
    with torch.no_grad():
        ...
        example["voxels"] = torch.zeros((example["voxels"].shape[0], example["voxels"].shape[1], 8),
        ...
        model(example)
        torch.onnx.export(model.reader, (example["voxels"], example["num_voxels"], example["coordinates"]),
                          "./pfe.onnx",
                          opset_version=11)

        rpn_input = torch.zeros((1, 64, 400, 800), dtype=torch.float32, device=gpu_device)
        torch.onnx.export(pp_model, rpn_input,
                          "./rpn.onnx",
                          opset_version=11)
    print("Done")
