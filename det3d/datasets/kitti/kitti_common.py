...
def create_kitti_info_file(data_path, save_path=None, relative_path=True):
    imageset_folder = Path(data_path).resolve() / "ImageSets"
    # imageset_folder = '/home/ubuntu/PycharmProjects/det3/CenterPoint/download/mykitti-origin/ImageSets/'
...
def get_pcd_path(idx, prefix, training=True, relative_path=True, exist_check=True):
    return get_kitti_info_path(
        idx, prefix, "velodyne", ".pcd", training, relative_path, exist_check
    )
...
def get_kitti_image_info(
    ...
    def map_func(idx):
        ...
        if label_info:
            ...
            annotations = get_mylabel_anno(label_path)
...
def get_mylabel_anno(label_path):
    annotations = {}
    annotations.update(
        {
            "name": [], #类别
            "occluded": [], #遮挡
            "dimensions": [], # dx, dy, dz length、width、height
            "location": [], # Xc, Yc, Zc center
            "alpha": [], # 朝向角 角度
            "rotation_y": [], #方位角 弧度
        }
    )
